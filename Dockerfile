...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends build-essential gfortran cmake libllvm-12-ocaml-dev libllvm12 llvm-12 llvm-12-dev llvm-12-doc llvm-12-examples llvm-12-runtime git gnupg curl lsb-release wget software-properties-common ca-certificates ssl-cert ffmpeg libsndfile1 -y
RUN python3 -m pip install --no-cache-dir scipy numpy
